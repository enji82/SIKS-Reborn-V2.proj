...
function tpg_getPerbaikanData(unitKerja, isAdmin) {
  try {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      result.push({
        id: String(data[i][0] || ""),
        unitKerja: String(rowUnit || ""),
        namaAsn: String(data[i][2] || ""),
        nip: String(data[i][3] || ""),
        statusPegawai: String(data[i][4] || ""),
        nuptk: String(data[i][5] || ""),
        jenisSK: String(data[i][6] || ""),
        tmt: data[i][7] ? Utilities.formatDate(new Date(data[i][7]), tz, "yyyy-MM-dd") : "",
        gajiPokok: data[i][8] || 0,
        noSktpg: String(data[i][9] || ""),
        bulanTpg: (data[i][10] instanceof Date) ? Utilities.formatDate(data[i][10], tz, "MMMM yyyy") : String(data[i][10] || ""),
        nilaiSalur: data[i][11] || 0,
        dokumenUrl: String(data[i][12] || ""),
        status: String(data[i][13] || "Diproses"),
        verifikasiOleh: String(data[i][14] || ""),
        waktuVerifikasi: data[i][15] ? Utilities.formatDate(new Date(data[i][15]), tz, "yyyy-MM-dd'T'HH:mm:ss") : "",
        keterangan: String(data[i][16] || ""),
        uploadOleh: String(data[i][17] || ""),
        waktuUpload: data[i][18] ? Utilities.formatDate(new Date(data[i][18]), tz, "yyyy-MM-dd'T'HH:mm:ss") : "",
        editOleh: String(data[i][19] || ""),
