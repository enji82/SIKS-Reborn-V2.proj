...
function getLastUploadedReportInfo(sheet, npsn, idxNpsn, idxBulan, idxTahun, idxStatus) {
  var lastRow = sheet.getLastRow();
  ...
  for (var i = 0; i < rangeValues.length; i++) {
    ...
    if (rowNpsn === targetNpsn) {
      ...
      var bIdx = -1;
      for (var j = 0; j < arrBulan.length; j++) {
        if (arrBulan[j].toLowerCase() === bStr.toLowerCase()) {
          bIdx = j;
          break;
        }
      }
      if (bIdx > -1 && tVal > 0) {
        var timeVal = (tVal * 12) + bIdx;
        if (timeVal > maxTimeVal) {
          maxTimeVal = timeVal;
          maxBulanStr = arrBulan[bIdx];
          maxTahunStr = String(tVal);
        }
      }
    }
  }
  ...
  if (maxTimeVal > -1) {
    return { bulan: maxBulanStr, tahun: maxTahunStr, timeVal: maxTimeVal };
  }
  return null;
}
    ...
    if (idxNpsn > -1 && idxBulan > -1 && idxTahun > -1) {
      ...
      if (lastRow >= 2) {
        ...
        if (!prevExists) {
            ...
            var targetBlnOut = targetBulanStr;
            var targetThnOut = targetTahun;
            if (lastRep) {
              ...
              targetBlnOut = arrBulan[nextBlnIdx];
              targetThnOut = nextThn;
            } else {
              msg += "Anda belum pernah mengisi laporan sebelumnya.<br>";
              msg += "Silakan isi laporan bulan Juli " + tahun + " terlebih dahulu.";
              targetBlnOut = "Juli";
              targetThnOut = parseInt(tahun);
            }
            return { success: false, message: msg, targetBulan: targetBlnOut, targetTahun: targetThnOut };
        } else {
            return { success: true, message: "Valid" };
        }
      }
...
function prosesSimpanLengkap(dbKey, namaSheet, source, form, fileData) {
  try {
    ...
    if (source === "PAUD") {
        ...
        sheet.getRange(newRowId, 1, 1, rowData.length).setValues([rowData]);
    }
